...
public void generateDataTable(Table table, StringBuilder builder, CSharpProjectOptions opts, Project prj, bool isClient, ushort tabLevel) {
	...
	if (!isClient && opts.enableEFExtensions) {
		builder.tabs(tabLevel).appendLine("public {2}partial class {0} : {1}IDatabaseMergeable<{0}>", table.name, opts.changeTracking ? "TrackingObject, " : (opts.uiBindings ? "BindingObject, " : string.init), (isClient || !opts.enableEFLazyLoading) ? "sealed " : string.init);
	} else if (opts.changeTracking) {
		builder.tabs(tabLevel).appendLine("public {2}partial class {0} : TrackingObject{1}", table.name, table.hasPrimaryKey ? "<" ~ table.name ~ ">" : string.init, (isClient || !opts.enableEFLazyLoading) ? "sealed " : string.init);
	} else if (opts.uiBindings) {
		builder.tabs(tabLevel).appendLine("public {1}partial class {0} : BindingObject", table.name, (isClient || !opts.enableEFLazyLoading) ? "sealed " : string.init);
	} else {
		builder.tabs(tabLevel).appendLine("public {1}partial class {0}", table.name, (isClient || !opts.enableEFLazyLoading) ? "sealed " : string.init);
	}
...
*/	}
	...
	foreach (fk; fkTarget) {
		...
		if (fk.direction != ForeignKeyDirection.OneToOne) {
			...
			builder.generateBindingMetadata(fk, opts, tabLevel, false, true);
			builder.tabs(tabLevel).appendLine("public {5}{0} {1} { get => {2}{3}; set => {4}; }", fk.sourceTable.getCSharpFullName(), fk.targetId(), getFieldName(fk.targetId()), opts.changeTracking ? ".Value" : string.init, generateSetter(getFieldName(fk.targetId()), opts, false), (isClient || !opts.enableEFLazyLoading) ? string.init : "virtual ");
		}
	}
	...
	foreach (fk; fkSource) {
		...
		if (fk.direction != ForeignKeyDirection.ManyToMany) {
			...
			builder.tabs(tabLevel).appendLine("public {5}{0} {1} { get => {2}{3}; set => {4}; }", fk.targetTable.getCSharpFullName(), fk.sourceId(), getFieldName(fk.sourceId()), opts.changeTracking ? ".Value" : string.init, generateSetter(getFieldName(fk.sourceId()), opts, false), (isClient || !opts.enableEFLazyLoading) ? string.init : "virtual ");
		}
		else {
			builder.generateBindingMetadata(fk, opts, tabLevel, true, false);
			if (opts.changeTracking) {
...
private void generateDataSqlMember(DataMember mm, StringBuilder builder, CSharpProjectOptions opts, bool isClient, ushort tabLevel)
{
	...
	if (opts.changeTracking) {
		...
		builder.tabs(tabLevel).appendLine("private {0} {1}{2};", getTypeFromSqlType(mm.sqlType, mm.isNullable), getFieldName(mm.name), (mm.hasDefault && mm.sqlType == SqlDbType.Bit) ? " = " ~ getMssqlDefaultValue(mm) : string.init);
	}
	builder.generateBindingMetadata(mm, opts, tabLevel, true);
